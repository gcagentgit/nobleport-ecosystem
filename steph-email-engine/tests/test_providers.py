...
from steph_email.providers import PROVIDERS, detect_provider, get_provider
...
def test_unknown_domain_is_generic():
    assert detect_provider("ops@nobleport.net").key == "generic"
...
def test_presets_are_complete():
    for key, p in PROVIDERS.items():
        ...
        assert p.imap_port == 993 and p.auth_methods
