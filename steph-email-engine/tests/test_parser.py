from steph_email.parser import html_to_text, normalize_subject, parse_message
from tests.conftest import make_raw


def test_parses_headers_body_and_date():
    p = parse_message(make_raw("Hi =?utf-8?q?caf=C3=A9?=", sender="Ann <ann@x.com>", body="hello world",
                               message_id="<m1@x>", date="Mon, 01 Sep 2026 10:00:00 -0400"))
    ...
    assert p["thread_key"] == "<m1@x>" and p["size"] > 0


def test_html_only_gets_text_and_attachments_listed():
    p = parse_message(make_raw("Plan", body="", html="<p>Site <b>plan</b> attached</p><script>x()</script>", attachment=("plan.pdf", b"%PDF-1.4")))
    assert "plan attached" in p["body_text"].lower() and "script" not in p["body_text"]
...
def test_thread_key_prefers_references_root_then_in_reply_to_then_subject():
    ...
    p1, p2 = parse_message(make_raw("Re: Change order 7")), parse_message(make_raw("FW: change order 7"))
