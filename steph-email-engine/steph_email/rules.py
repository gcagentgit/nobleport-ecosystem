...
Keyword lists matched (case-insensitive) against subject, sender and body.
Override or extend with ``<data_dir>/rules.json`` shaped ``{"tag": ["kw", ...]}``.
"""
...
DEFAULT_RULES: dict[str, list[str]] = {
    "permit": ["permit", "building department", "inspection", "certificate of occupancy", "zoning", "variance", "inspector"],
    ...
    "safety": ["osha", "safety", "incident", "injury", "hazard", "stop work"],
    "supplier": ["delivery", "purchase order", "backorder", "material", "lumber", "concrete"],
    "legal": ["attorney", "counsel", "lawsuit", "litigation", "subpoena", "notice of", "lien"],
    "urgent": ["urgent", "asap", "immediately", "deadline", "final notice", "past due", "time sensitive"],
    "newsletter": ["unsubscribe", "newsletter", "webinar", "% off", "promo code", "view in browser"],
}
...
_SENDER_RULES: dict[str, list[str]] = {
    ...
    "newsletter": ["noreply", "no-reply", "donotreply", "newsletter", "marketing@", "mailer"],
}
...
class Tagger:
    ...
    def load(cls, data_dir: Path) -> "Tagger":
        ...
        if path.exists():
            ...
            return cls({**DEFAULT_RULES, **{k: list(v) for k, v in custom.items()}})
        return cls()
